...
python -m perfume_trend_sdk.jobs.aggregate_daily_market_metrics --date "$TODAY"

# Step 3: Detect signals
echo "[pipeline] Step 3 — Signal detection for date=$TODAY"
python -m perfume_trend_sdk.jobs.detect_breakout_signals --date "$TODAY"

# Step 4: Verify state (non-blocking — log failures but don't abort)
echo "[pipeline] Step 4 — Market state verification"
python -m perfume_trend_sdk.jobs.verify_market_state || \
