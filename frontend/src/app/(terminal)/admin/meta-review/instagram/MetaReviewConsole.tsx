...
// ---------------------------------------------------------------------------
// Helper components (defined outside MetaReviewConsole — stable references)
// ---------------------------------------------------------------------------
...
export function MetaReviewConsole() {
  ...
  // Demo state machine — single source of truth
  ...
  async function runDemo() {
    ...
    try {
      const resp = await fetch(
        `/api/admin/instagram-review?action=demo&hashtag=${encodeURIComponent(
          selectedHashtag
        )}`
      );
      ...
      try {
        ...
        setDemoError(
          `Response could not be parsed (HTTP ${resp.status}). Check backend logs.`
        );
        setDemoState("error");
        return;
      }
  ...
  // Items derived safely — never null/undefined regardless of API shape
  const safeItems: IGMediaItem[] =
    demoResult && Array.isArray(demoResult.items) ? demoResult.items : [];
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-10 space-y-8">
      ...
      <section className="rounded border border-zinc-800 bg-zinc-900/50 p-5 space-y-3">
        ...
        <ul className="text-xs text-zinc-400 space-y-1.5 leading-relaxed list-none">
          <li>
            ...
            analyze aggregated trend signals from public social media content to
            help brands, marketers, and researchers understand which fragrances
          ...
          <li>
            ...
            post caption text, timestamp, permalink, and media type.
          </li>
          <li>
            ...
            users, resell raw content datasets, or expose raw post text on
            public pages. All data is processed internally into aggregated trend
            intelligence.
          </li>
          <li>
            <span className="text-zinc-300 font-medium">Output:</span> Fragrance
            trend scores, momentum indicators, and market signals — surfaced in
            the authenticated terminal for industry professionals.
          </li>
        </ul>
      </section>
      ...
      {/* ── Demo result area (always present when not idle) ──────────────── */}

      {demoState === "loading" && (
        <section className="rounded border border-zinc-800 bg-zinc-900/30 p-4 flex items-center gap-3">
      ...
      {demoState === "error" && demoError && (
        <section className="rounded border border-rose-700/60 bg-rose-950/30 p-4 space-y-2">
      ...
      {demoState === "error" && !demoError && (
        <section className="rounded border border-rose-700/60 bg-rose-950/30 p-4">
          <p className="text-xs text-rose-300">
            Demo failed with no error detail. Check backend logs.
          </p>
        </section>
      )}
          ...
          {safeItems.length > 0 ? (
            ...
            <div className="rounded border border-zinc-800 bg-zinc-900/30 p-4">
              <p className="text-xs text-zinc-500">
                Hashtag resolved successfully (ID:{" "}
                <span className="font-mono">{demoResult.hashtag_id}</span>),
                but no public posts were returned from the{" "}
                <code className="text-zinc-400">recent_media</code> endpoint.
                The hashtag may have limited recent activity.
              </p>
            </div>
          )}
      ...
      {demoState === "success" && !demoResult && (
        <section className="rounded border border-zinc-700/40 bg-zinc-900/30 p-4">
          <p className="text-xs text-zinc-500">
            Request completed but response was empty. Check backend logs.
          </p>
        </section>
      )}
      ...
      <section className="rounded border border-zinc-800 bg-zinc-900/50 p-5 space-y-3">
        ...
        <dl className="space-y-2 text-xs">
          ...
          <div>
            ...
            <dd className="text-zinc-600 font-mono mt-0.5">
              GET
              /v21.0/&#123;hashtag_id&#125;/recent_media?user_id=&#123;id&#125;&fields=id,caption,timestamp,permalink,media_type
            </dd>
          </div>
        </dl>
        <p className="text-[11px] text-zinc-600">
