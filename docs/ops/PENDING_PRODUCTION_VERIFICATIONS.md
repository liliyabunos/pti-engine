...
| PV-008 | SIG-QA2 shadow mode observation — migrations 052+053, evidence gate, weak_evidence_log | `IMPLEMENTED — SHADOW MODE PENDING PRODUCTION OBSERVATION` | PV-008-B1 RESOLVED (f067364). RES-AMB-FIVE RESOLVED (1678158). RES-AMB-MENSCOL RESOLVED (3fbf455). **PV-008-B2 RESOLVED** — PV-008-B2-FIX1 implemented 2026-05-19. **Counter: 0/5 minimum floor**. 2026-05-20 23:00 UTC evening pipeline: COMPLETED (recorded_at 23:14:55 UTC) but Run #1 NOT COUNTED — same root cause as morning: all 61 WEL rows pre-created at 09:02 UTC; scheduled pipeline wrote 0 new rows (ON CONFLICT DO UPDATE only). suppress_rate=62.3% from 61 pre-fill rows. No material anomalies; known-good passes (Creed Aventus EDP, Dior Sauvage) confirmed working. Known standalone-brand suppressions (CDNI/M, BR540) within expected shadow behavior. **Next qualifying run: 2026-05-21 11:00 UTC morning — May 21 has 0 WEL rows; first aggregation will create fresh rows. No manual ops on May 21 data before 11:00 UTC.** |
| OPS-CRON-01 | Pipeline scheduling gap 2026-05-17 through 2026-05-20 (Railway outage) | `CRON OPERATIONALLY RESTORED — OPS-CRON-PIPELINE-GAP OPEN (PV-008 Run #1 not counted)` | Root causes: (1) deploys during cron windows; (2) SIG-QA2 UUID crash; (3) Railway outage killed May 19 23:00 UTC cron. May 17/18/19 data: fully recovered. May 20 11:00 UTC cron: COMPLETED (WARNING, 201 reddit + 265 YT, 27 signals, 11:23:29 UTC). May 20 23:00 UTC cron: COMPLETED (WARNING, same values as morning, 23:14:55 UTC) — 0 new content items collected (evening ingest returned items already in DB); 0 new WEL rows created; aggregated against pre-existing May 20 state. OPS-CRON-PIPELINE-GAP remains OPEN: requires PV-008 Run #1/5 confirmed — first qualifying scheduled run with fresh WEL rows. **Next qualifying run: 2026-05-21 11:00 UTC morning pipeline.** |
