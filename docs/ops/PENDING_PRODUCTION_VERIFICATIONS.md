...
| PV-008 | SIG-QA2 shadow mode observation — migrations 052+053, evidence gate, weak_evidence_log | `BLOCKED — SIG-QA2-STANDALONE-BRAND-REPAIR IN PROGRESS` | **Founder decision 2026-05-21:** PV-008 does not continue toward activation readiness while standalone-brand recognition issue remains unresolved. Repair approved: SIG-QA2-STANDALONE-BRAND-REPAIR (D6 dimension + alias_used population). **Counter reset to 0/5. No runs from before this repair count.** First qualifying run: first scheduled cron after this repair deploys. See PV-008-SBR entry below. |
| PV-008-SBR | SIG-QA2-STANDALONE-BRAND-REPAIR — D6 high-recognition standalone alias modifier + alias_used resolver fix | `IMPLEMENTED — AWAITING PIPELINE VERIFICATION` | Commit: `pending deploy` — D6 dimension added to evidence_scorer.py; alias_used populated in perfume_resolver.py; 119/119 tests pass. Prerequisites: (1) deploy to Railway; (2) first scheduled cron creates WEL rows; (3) confirm avatns/CDNI/M/BR540 pass gate (would_suppress=False) in WEL output; (4) confirm B2-FIX1 cap still fires for Ultimate Man. After verified: PV-008 counter restarts from 0 with new scorer. |
| OPS-CRON-01 | Pipeline scheduling gap 2026-05-17 through 2026-05-20 (Railway outage) | `CRON OPERATIONALLY RESTORED — OPS-CRON-PIPELINE-GAP SUSPENDED (PV-008 blocked by SBR repair)` | Root causes: (1) deploys during cron windows; (2) SIG-QA2 UUID crash; (3) Railway outage killed May 19 23:00 UTC cron. May 17–19 data: fully recovered. May 20 cron: COMPLETED. OPS-CRON-PIPELINE-GAP remains OPEN: awaiting PV-008 counter restart after SIG-QA2-STANDALONE-BRAND-REPAIR deploys. |
